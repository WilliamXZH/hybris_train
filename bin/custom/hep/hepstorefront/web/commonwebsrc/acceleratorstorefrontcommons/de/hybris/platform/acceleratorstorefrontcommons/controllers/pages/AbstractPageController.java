...
/**
 * Base controller for all page controllers. Provides common functionality
 * for all page controllers.
 */
public abstract class AbstractPageController extends AbstractController {
    ...
    public static final String REDIRECT_TO_LOGIN_FOR_CHECKOUT =
			REDIRECT_PREFIX + "/login/checkout";
    public static final String REDIRECT_TO_MULTISTEP_CHECKOUT =
			REDIRECT_PREFIX + "/checkout/multi";
    ...
    protected void storeCmsPageInModel(final Model model, final
        ...
        if (model != null && cmsPage != null) {
            ...
            if (cmsPage instanceof ContentPageModel) {
                storeContentPageTitleInModel(model, getPageTitleResolver()
						.resolveContentPageTitle(cmsPage.getTitle()));
            }
        }
    }

    protected void storeContentPageTitleInModel(final Model model, final
	String title) {
    ...
    protected String getViewForPage(final Model model) {
        if (model.containsAttribute(CMS_PAGE_MODEL)) {
            final AbstractPageModel page = (AbstractPageModel) model.asMap()
					.get(CMS_PAGE_MODEL);
            if (page != null) {
    ...
    protected String getViewForPage(final AbstractPageModel page) {
        if (page != null) {
            ...
            if (masterTemplate != null) {
                final String targetPage = cmsPageService
						.getFrontendTemplateName(masterTemplate);
    ...
    /**
     * Checks request URL against properly resolved URL and returns null if
     * url is proper or redirection string if not.
     *
     * @param request         - request that contains current URL
     * @param response        - response to write "301 Moved Permanently"
     *                           status to if redirected
     * @param resolvedUrlPath - properly resolved URL
     * @return null if url is properly resolved or redirection string if not
     * @throws UnsupportedEncodingException
     */
    protected String checkRequestUrl(final HttpServletRequest request, final
	HttpServletResponse response,
                                     final String resolvedUrlPath) throws
			UnsupportedEncodingException {
        final String resolvedUrl = response.encodeURL(request.getContextPath
				() + resolvedUrlPath);
        final String requestURI = UriUtils.decode(request.getRequestURI(),
				StandardCharsets.UTF_8.toString());
        final String decoded = UriUtils.decode(resolvedUrl, StandardCharsets
				.UTF_8.toString());
        if (StringUtils.isNotEmpty(requestURI) && requestURI.endsWith
				(decoded)) {
            return null;
        } else {
            //  org.springframework.web.servlet.View
			// .RESPONSE_STATUS_ATTRIBUTE = "org.springframework.web.servlet
			// .View.responseStatus"
            request.setAttribute("org.springframework.web.servlet.View" +
    ...
    protected ContentPageModel getContentPageForLabelOrId(final String
																  labelOrId)
    ...
    protected void storeContinueUrl(final HttpServletRequest request) {
        ...
        getSessionService().setAttribute(WebConstants.CONTINUE_URL, url
				.toString());
    }

    protected void setUpMetaData(final Model model, final String
			metaKeywords, final String metaDescription) {
    ...
    protected MetaElementData createMetaElement(final String name, final
	String content) {
    ...
    protected void setUpMetaDataForContentPage(final Model model, final
	ContentPageModel contentPage) {
        setUpMetaData(model, contentPage.getKeywords(), contentPage
				.getDescription());
    }

    protected RequestContextData getRequestContextData(final
													   HttpServletRequest
															   request) {
     ...
     * @param response object represents http response needed for setting not
     *                    found status.
     * @throws CMSItemNotFoundException when cannot find ContentPage with
     * {@value #ERROR_CMS_PAGE} id
     */
    ...
    /**
     * Method used to determine the checkout redirect URL that will handle the
     * checkout process.
     *
